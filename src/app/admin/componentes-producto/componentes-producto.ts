...
Math: any;
  ...
  applyFilters(): void {
    ...
    this.totalPages = Math.ceil(this.totalItems / this.itemsPerPage);
    const startIndex = (this.currentPage - 1) * this.itemsPerPage;
    this.filteredComponentes = filtered.slice(startIndex, startIndex + this.itemsPerPage);
  }
  ...
  sort(field: string): void {
    ...
    toggleSelect(pieza: ComponentesProducto): void {
      const index = this.selectedComponentesProducto.findIndex((p) => p.id === pieza.id);
      if (index > -1) {
  ...
  saveComponente() {
    ...
    if (formValue.id) {
      this.apiService.updateComponenteProducto(formValue.id, formValue).subscribe({
        next: () => {
  ...
  this.apiService.deleteComponenteProducto(this.componenteToDelete.id).subscribe({
    next: () => {
